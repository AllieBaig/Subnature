...
export default function AudioEngine() {
  ...
  const generateToneBlob = async (type: string, options: any) => {
    ...
    if (type === 'pureHz' || type === 'solfeggio' || type === 'schumann') {
      ...
      // Apply frequency shaping filter to noise
      const filter = ctx.createBiquadFilter();
      filter.type = 'peaking';
      filter.frequency.setValueAtTime(options.frequency || 432, 0);
      filter.Q.setValueAtTime(2.0, 0); // Natural sharpness
      filter.gain.setValueAtTime(8, 0); // Noticeable but gentle boost
      
      // Gain calculation (Safety is already handled by masterGainNode)
      const baseGain = options.noiseType === 'white' ? 0.05 : 0.08;
      gain.gain.setValueAtTime(baseGain, 0); 
      
      source.buffer = buffer;
      source.loop = true;
      
      // Pitch Safe Mode for Noise: Add sub-bass resonance if enabled
      if (options.pitchSafeMode) {
        const subGain = ctx.createGain();
      ...
      try {
        ...
        filter.type = 'peaking';
        filter.frequency.setValueAtTime(options.frequency || 432, 0);
        filter.Q.setValueAtTime(1.5, 0); // Wider for natural sounds
        filter.gain.setValueAtTime(6, 0); // Subtle tonal coloring
        ...
        // Pitch Safe Mode for Nature: Add grounding harmonic
        if (options.pitchSafeMode) {
          const subOsc = ctx.createOscillator();
          const subGain = ctx.createGain();
          subOsc.type = 'sine';
          subOsc.frequency.setValueAtTime((options.frequency || 432) / 4, 0);
          subGain.gain.setValueAtTime(0.01, 0);
          subOsc.connect(subGain);
          subGain.connect(masterGainNode);
          subOsc.start(0);
        }
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      syncBgLayer('noise', isLayerPlaying, 'noise', { 
    ...
    sync();
  }, [isPlaying, settings.noise.isEnabled, settings.noise.playInBackground, settings.noise.type, settings.noise.frequency, settings.noise.pitchSafeMode, settings.noise.volume, settings.noise.gainDb, settings.fadeInOut]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      await syncBgLayer('nature', isLayerPlaying, 'nature', { 
        url: sound?.url || '',
        frequency: settings.nature.frequency,
        pitchSafeMode: settings.nature.pitchSafeMode
      }, settings.nature.volume, settings.nature.gainDb);
    };
    sync();
  }, [isPlaying, settings.nature.isEnabled, settings.nature.playInBackground, settings.nature.type, settings.nature.frequency, settings.nature.pitchSafeMode, settings.nature.volume, settings.nature.gainDb, settings.nature.normalize, settings.fadeInOut, safetyMultiplier]);
